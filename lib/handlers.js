    ...
    _data.read("users", phone, function (err, userData) {
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          // if valid, create a new token with a random name. Set the expiration data to one hour in future
          ...
          if (!err && userData) {
            const userChecks =
              ...
              userData.checks instanceof Array
                ? userData.checks
                : [];
            // Verify that the user has less the max number of checks
            if (userChecks.length < config.maxChecks) {
              ...
              callback(400, {
                Error: `The user already reach the max number of checks (${config.maxChecks})`,
              });
            }
          } else {
            callback(403);
                      ...
                      if (!err && userData) {
                        const userChecks =
                          ...
                          userData.checks instanceof Array
                            ? userData.checks
                            : [];
                        // Remove the deleted check from the list of the checks
                        ...
                        callback(500, {
                          Error:
                            "Could not find the user who created the check and so could not delete the check",
                        });
                      }
                    }
                  );
